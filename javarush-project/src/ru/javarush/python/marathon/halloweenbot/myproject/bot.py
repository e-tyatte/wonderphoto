...
async def edit_message(update, context):
    ...
    if not os.path.exists(photo_path):
        await send_text(update, context, 'Сначала загрузите или создайте картинку')
        return
...
async def merge_button(update, context):
    ...
    if image_count <2:
        await send_text(update, context, f'Сначала загрузите ваши фото')
        return
...
async def party_command(update, context):
    ...
    await send_text_buttons(update, context, text, {
        ...
        "party_image4": "🧟‍♀️ Гниение времени (зомби)",
...
async def party_button(update, context):
    ...
    if not os.path.exists(photo_path):
        await send_text(update, context, f'Сначала загрузите ваше фото')
        return
...
async def video_button(update, context):
    ...
    prompt = load_prompt(query)
    await send_text(update, context, f'Генерация видео займёт около 20 секунд')
